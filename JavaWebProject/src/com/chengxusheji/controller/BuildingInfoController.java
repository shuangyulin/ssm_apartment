...
public class BuildingInfoController extends BaseController {
	...
	/*ajax方式按照查询条件分页查询宿舍信息信息*/
	@RequestMapping(value = { "/list" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void list(String buildingName,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (buildingName == null) buildingName = "";
		if(rows != 0)buildingInfoService.setRows(rows);
		List<BuildingInfo> buildingInfoList = buildingInfoService.queryBuildingInfo(buildingName, page);
	    /*计算总的页数和总的记录数*/
	    buildingInfoService.queryTotalPageAndRecordNumber(buildingName);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String buildingName,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (buildingName == null) buildingName = "";
		List<BuildingInfo> buildingInfoList = buildingInfoService.queryBuildingInfo(buildingName, currentPage);
	    /*计算总的页数和总的记录数*/
	    buildingInfoService.queryTotalPageAndRecordNumber(buildingName);
	    /*获取到总的页码数目*/
	    int totalPage = buildingInfoService.getTotalPage();
	...
	public String delete(@PathVariable Integer buildingId,HttpServletRequest request) throws UnsupportedEncodingException {
		  try {
			  buildingInfoService.deleteBuildingInfo(buildingId);
	            request.setAttribute("message", "宿舍信息删除成功!");
	            return "message";
	        } catch (Exception e) { 
	            e.printStackTrace();
	            request.setAttribute("error", "宿舍信息删除失败!");
				return "error";
	...
	/*按照查询条件导出宿舍信息信息到Excel*/
	@RequestMapping(value = { "/OutToExcel" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void OutToExcel(String buildingName, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(buildingName == null) buildingName = "";
        List<BuildingInfo> buildingInfoList = buildingInfoService.queryBuildingInfo(buildingName);
